...
function UI.DrawSpriteSimple(textureDict, textureName, screenX, screenY, width, height, heading, red, green, blue, alpha, settings, cb)
    local pos
    if alpha <= 0 and not settings.drawEvenIfAlpha0 ~= nil and settings.drawEvenIfAlpha0 == false then
        return
    else
...
function UI.DrawRectSimple(screenX, screenY, width, height, heading, red, green, blue, alpha, settings, cb)
    alpha = math.floor(alpha)
...
function UI.CalculateLinearScalablePosition(targetPosition, actualPosition, speed)
    ...
    if targetPosition > actualPosition then
        if actualPosition + (speed * 1) > targetPosition then
            return targetPosition
        else
            return actualPosition + (speed * 1)
        end
    else
        if actualPosition - (speed * 1) < targetPosition then
            return targetPosition
